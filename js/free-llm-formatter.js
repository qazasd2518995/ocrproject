...
class FreeLLMInvoiceFormatter {
    ...
    createPrompt(rawText) {
        return `請分析以下發票資料，這包含了表格結構資訊和原始OCR文字。請正確提取商品項目。
...
⚠️ 特別注意：
1. 如果看到「產品行:」標記，這些是經過表格結構分析的真實商品行
2. 產品編號格式：字母+數字組合（如：CC104-4001、A1234-567）
3. 品名規格：產品的完整中文描述
4. 請忽略純數字（價格、數量等）
5. 每個產品行通常包含：編號 + 品名規格 + 其他資訊
6. 從每行中提取編號和品名，忽略數量、價格
