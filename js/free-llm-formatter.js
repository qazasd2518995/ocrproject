...
class FreeLLMInvoiceFormatter {
    ...
    createPrompt(rawText) {
        return `請仔細分析以下OCR辨識的發票表格文字，正確識別表格結構並提取商品資訊。
...
⚠️ 重要識別規則：
1. 這是一個表格格式的發票，有「產品編號」和「品名」等欄位
2. 請仔細區分表格的欄位結構，不要將數量、價格等其他欄位誤認為產品編號
3. 產品編號通常是字母+數字的組合（如：CC104-4001, A1234-567等）
4. 品名規格是產品的完整描述文字
5. 避免將價格數字（如：102.8、8903等純數字）誤認為產品編號
6. 一個表格行只對應一個商品項目，不要重複計算

請按照以下要求整理：
1. 仔細分析表格結構，識別真正的商品項目
